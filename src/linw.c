 ...
 *
 * Edits to original lin.c code to create this new linw.c code:
...
void linw(double *exp_zbeta, double *time, int *status, double *covar,
         double *prev_exp_zbeta, double *M, int *n_row, int *n_col,
         int *a_type, double *sweight, double *result) {
    ...
    for (j=0; j<*n_col; j++) {
        for (i=0; i<*n_row; i++) {
            ...
            if ( status[i] != 0 ) {
               ...
               sum2 = 0;
               for (k=i; k<*n_row; k++) {
                   tmp = A(time[i], prev_exp_zbeta[k], *M) * exp_zbeta[k] * sweight[k];
                   sum1 += tmp;
                   sum2 += z[j][k]*tmp;
               }
               ...
               res[j][i] = ( z[j][i] - sum2/sum1 )*a_ii*sweight[i];
               tmp1[i]   = ( (a_ii * exp_zbeta[i] * sweight[i]) / (sum1*sum1) ) * 
                           (sum2 - z[j][i]*sum1);
        ...
        for (i=0; i<*n_row; i++) {
            for (k=i; k<*n_row; k++) {
                res[j][i] -= A(time[i], prev_exp_zbeta[k], *M) * sweight[i] * tmp1[k];
            }
        }
