 ...
 *
 * Edits to original re.c code to create this new rew.c code:
...
void rew(double *beta, double *time, int *status, double *covar,
        double *prev_exp_zbeta, double *M, int*n_row, int *n_col,
        int *a_type, double *sweight, double *res, double *gradient, double *hessian) {
    ...
    for (i=*n_row-1; i>=0; i--) {
        ...
        if ( status[i] != 0 ) {
            ...
            for (ii=i; ii<*n_row; ii++) {
                ...
                tmp = A(time[i], prev_exp_zbeta[ii], *M)*exp_zbeta[ii]*sweight[ii];
                sum1 += tmp;
                for (j=0; j<*n_col; j++) {
                    sum2[j] += z[j][ii] * tmp;
                    for (k=0; k<=j; k++) {
            ...
            *res += a_ii*(log(sum1) - zbeta[i])*sweight[i];
    
            for (j=0; j<*n_col; j++) {
                tmp = sum2[j]/sum1;
                gradient[j] += (tmp - z[j][i])*a_ii*sweight[i];
    
                for (k=0; k<=j; k++) {
                    hess[j][k] += ( (sum3[j][k] - tmp*sum2[k])/sum1 ) * a_ii * sweight[i];
                }
            }
