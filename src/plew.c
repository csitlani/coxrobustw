 ...
 *
 * Edits to original ple.c code to create this new plew.c code:
...
void plew(double *beta, double *time, int *status, double *covar, int *n_row,
         int *n_col, double *sweight, double *res, double *gradient, double *hessian) {
    ...
    *res = 0;
    ...
    for (i=*n_row-1; i>=0; i--) {
        ...
        sum1 += exp_zbeta * sweight[i];
        for (j=0; j<*n_col; j++) {
            sum2[j] += z[j][i] * exp_zbeta * sweight[i];
            for (k=0; k<=j; k++) {
                sum3[j][k] += z[j][i] * z[k][i] * exp_zbeta * sweight[i];
            }
        }
    
        if ( status[i] != 0 ) {
    
            *res += (log(sum1) - zbeta) * sweight[i];
    
            for (j=0; j<*n_col; j++) {
    
                tmp = sum2[j]/sum1;
                gradient[j] += (tmp - z[j][i]) * sweight[i];
    
                for (k=0; k<=j; k++) {
                    hess[j][k] += ((sum3[j][k] - tmp*sum2[k])/sum1) * sweight[i];
                }
            }
