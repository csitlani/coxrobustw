...
coxrw.fit <- function(x, y, trunc, init, f.weight, singular.ok, weight) {
    ...
    res      <- coxr.ple.weighted(rep(0, ncol), time, status, z, nrow, ncol, sweight) # returns beta, hess, hessinv
    beta     <- res$beta
    beta.ple <- beta

    prev_ezbeta <- exp(z %*% beta)
    rescov <- ple.covar.robust(beta, time, status, z, prev_ezbeta, nrow, ncol, res$hessinv, sweight)
    cv.ple   <- diag(1/maxabs, ncol = ncol, nrow=ncol) %*% rescov$covar %*%
                diag(1/maxabs, ncol = ncol, nrow=ncol)
    tmp <- sqrt(nrow)*as.matrix(beta-init*maxabs)
    wald.test <- t(tmp) %*% chol2inv(chol(rescov$covar*nrow)) %*% tmp
    ...
    res <- coxr.re.weighted(rep(0, ncol), lmb, status, z, prev_ezbeta, M, nrow, ncol, f.weight, sweight)
    beta <- res$beta

    res <- coxr.covar.weighted(beta, lmb, status, z, prev_ezbeta, M, nrow, ncol, f.weight, res$hessinv, sweight)
       ...
       res <- .C("ple", as.double(beta), as.double(time),
        ...
        if ( norm <= eps ) {
            ...
            res <- .C("ple", as.double(beta), as.double(time),
                    as.integer(status), as.double(z), as.integer(nrow),
                    as.integer(ncol), res = double(1), gradient = double(ncol),
                    hessian = double(ncol*ncol), PACKAGE = "coxrobustw")
...
coxr.ple.weighted <- function(beta, time, status, z, nrow, ncol, sweight) {
    ...
    for (i in 1:iter.max) {
       ...
       res <- .C("plew", as.double(beta), as.double(time),
                as.integer(status), as.double(z), as.integer(nrow),
                as.integer(ncol), as.double(sweight), res = double(1), gradient = double(ncol),
                hessian = double(ncol*ncol))
...
ple.covar.robust <- function(beta, time, status, z, prev_exp_zbeta, nrow, ncol,
                  hessinv, sweight) {

    cres <- .C("plewcov", as.double( exp(z %*% beta) ), as.double(time),
              as.integer(status), as.double(z), as.double(prev_exp_zbeta),
              as.integer(nrow), as.integer(ncol),
              as.double(sweight), lin = double(nrow * ncol),
              PACKAGE = "coxrobustw")

    lin <- matrix(cres$lin, nrow, ncol)

    IF <- (lin %*% -hessinv) * sweight   #extra sweight is because it's not included in lin
...
coxr.re.weighted <- function(beta, time, status, z, prev_ezbeta, M, nrow, ncol, f.weight, sweight) {
    ...
    for (i in 1:iter.max) {
        ...
        res <- .C("rew", as.double(beta), as.double(time), as.integer(status),
                as.double(z), as.double(prev_ezbeta), as.double(M),
                as.integer(nrow),  as.integer(ncol), as.integer(f.weight), as.double(sweight),
                res = double(1),  gradient = double(ncol),
                hessian  = double(ncol*ncol))
...
coxr.covar.weighted <- function(beta, time, status, z, prev_exp_zbeta, M, nrow, ncol,
                  f.weight, hessinv, sweight) {

    cres <- .C("linw", as.double( exp(z %*% beta) ), as.double(time),
              as.integer(status), as.double(z), as.double(prev_exp_zbeta),
              as.double(M), as.integer(nrow), as.integer(ncol),
              as.integer(f.weight), as.double(sweight), lin = double(nrow * ncol),
              PACKAGE = "coxrobustw")
